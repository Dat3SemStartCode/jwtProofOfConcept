...
import jwtdemo.exceptions.GenericExceptionMapper;
...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
 ...
 public void filter(ContainerRequestContext request) throws IOException {
   if (isSecuredResource()) {
     ...
     if (token == null) {
       request.abortWith(GenericExceptionMapper.makeErrRes(token, 403));
       return;
     }
     try {
       ...
       request.abortWith(GenericExceptionMapper.makeErrRes("Token not valid (timed out?)", 403));
     }
   }
 }
